...
Este documento describe cómo reportar vulnerabilidades en **AuthApp CI** y qué prácticas de seguridad seguimos en el repositorio.
...
| Última release (tag)    | Sí                            |
| Releases anteriores     | Solo la última versión estable recibe correcciones |
...
- Fija las versiones de dependencias en el `pom.xml` (Maven resuelve versiones exactas y las bloquea en el build).
- Auditoría de vulnerabilidades: aún no configurada. Se recomienda integrar [OWASP Dependency-Check](https://owasp.org/www-project-dependency-check/) en el pipeline (`mvn org.owasp:dependency-check-maven:check`).
